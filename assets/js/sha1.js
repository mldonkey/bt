...
function sha1File(settings)
{
    ...
    {
        var end = Math.min(inEnd, file.length);
        var start = inStart;
        var blob = file.slice(start, end);
        do{
            file.sha1_progress = (end * 100 / file.length);
            var block = Crypto.util.bytesToWords( new Uint8Array(str2buf(blob)));

            if (end === file.length)
            {
                var bTotal, bLeft, bTotalH, bTotalL;
                bTotal = file.length * 8;
                bLeft = (end - start) * 8;
                ...
                var sha1_hash = Crypto.util.bytesToHex(Crypto.util.wordsToBytes(hash));
